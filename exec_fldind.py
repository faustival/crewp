...
'''
rootpath = '/home/jinxi/pwjobs/chrgden_field'
os.chdir(rootpath)
...
for slab in slab_plotlist:
    ...
    # iterating plot over fields
    for i in range(0, len(slab.flds)-1):
        legend_pref = slab.elem + slab.ort + 'f={0:.2f}'.format(slab.flds[i+1])
        ax_diff_tot.plot(slab.zaxis, -slab.chrgz_diffld[i][0], color=color, label=legend_pref+' Total')
        ax_diff_free.plot(slab.zaxis, -slab.chrgz_diffld[i][1], color=color, linestyle='--', dashes=(5,1.5), label=legend_pref+' $d$')
        ax_diff_free.plot(slab.zaxis, -slab.chrgz_diffld[i][2], color=color, label=legend_pref+' Free')
        z0 = slab.imgplane[i]
        for ax in ax_diff:
